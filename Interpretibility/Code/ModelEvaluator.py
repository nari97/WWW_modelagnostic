...
from AugmentedKGE.Train.Evaluator import Evaluator
...
def get_model(model_name, dataset_name, ent_total, rel_total, dim=0, dim_e=0, dim_r=0):
    if model_name == "TransE":
        model = KelpieTransE(ent_total, rel_total, dim)
        neg_predict = True

    if model_name == "ComplEx":
        model = KelpieComplEx(ent_total, rel_total, int(dim / 2))
        neg_predict = True

    if model_name == "ConvE":
        model = KelpieConvE(ent_total, rel_total, dim)
        neg_predict = True
...
def materialize_individual(dataset_name, model_name, use_gpu=False):
    ...
    results_folder_path = "../../Interpretibility/Results/Materializations/" + dataset_name + "/" + model_name
    print("Starting Materializer")
    start = time.perf_counter()
    evaluator = Evaluator(manager=manager)

    collector = evaluator.evaluate(model=akge_model)
...
if __name__ == '__main__':
    materialize_individual(model_name="TuckER", dataset_name="WN18RR", use_gpu=False)
